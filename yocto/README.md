...
│   └── yolo-update.mjs          # A/B partition updater + fast deploy
...
### Fast Deploy (App-Only)

After a device is flashed and reachable over SSH (`inky@inky-soup.local`), use the YOLO updater in fast mode to deploy only the server binary + UI assets (no reboot):
...
Notes:
- Uses your saved preferences from `.flash-config.json` (SSH key + machine type).
- Syncs `upload-server/templates` and `upload-server/static/js` to `/usr/share/inky-soup` and updates `/usr/bin/inky-soup-server`.
- Does **not** touch `/data/inky-soup/images` and does **not** reboot.

### Full A/B Update (Rootfs)

To stream a full root filesystem update to the inactive A/B slot and reboot into it:
